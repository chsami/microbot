...
public class FletchingScript extends Script {
    ...
    public void run(FletchingConfig config) {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            if (config.Afk() && Random.random(1, 100) == 2)
                sleep(1000, 60000);
            try {
                String primaryItemToFletch = config.fletchingMode().getItemName();
                String secondaryItemToFletch = config.fletchingMode() == FletchingMode.STRUNG
                        ? config.fletchingMaterial().getName() + " " + config.fletchingItem().getContainsInventoryName() + " (u)"
                        : config.fletchingMaterial().getName();
                boolean hasRequirementsToFletch = Inventory.hasItem(primaryItemToFletch)
                        && Inventory.hasItemAmount(secondaryItemToFletch, config.fletchingItem().getAmountRequired());
                boolean hasRequirementsToBank = !Inventory.hasItem(primaryItemToFletch)
                        || !Inventory.hasItemAmount(secondaryItemToFletch, config.fletchingItem().getAmountRequired());

                if (hasRequirementsToFletch) {
                    fletch(config, primaryItemToFletch, secondaryItemToFletch);
                }
                if (hasRequirementsToBank) {
                    bankItems(config, primaryItemToFletch, secondaryItemToFletch);
                }
    ...
    private void bankItems(FletchingConfig config, String primaryItemToFletch, String secondaryItemToFletch) {
        Rs2Bank.openBank();
        ...
        if (!Rs2Bank.hasItem(config.fletchingMaterial().getName())) {
            Rs2Bank.closeBank();
            Microbot.status = "[Shutting down] - Reason: " + config.fletchingMaterial().getName() + " not found in the bank.";
            Microbot.getNotifier().notify(Microbot.status);
            shutdown();
            return;
        }
        Rs2Bank.withdrawItemsAll(secondaryItemToFletch);
        sleepUntilOnClientThread(() -> Inventory.hasItem(secondaryItemToFletch));
        sleep(600, 3000);
        Rs2Bank.closeBank();
    }
    ...
    private void fletch(FletchingConfig config, String itemToFletchWith, String logsToFletch) {
        ...
        sleepUntilOnClientThread(() -> Rs2Widget.getWidget(17694736) == null);
        sleepUntilOnClientThread(() -> !Inventory.hasItemAmount(logsToFletch, config.fletchingItem().getAmountRequired()), 60000);
    }
