    ...
    public boolean run(WineConfig config) {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            try {
                ...
                if (Rs2Inventory.getAmountForItem("grapes") > 0 && (Rs2Inventory.getAmountForItem("jug of water") > 0)) {
                    Rs2Inventory.useItemOnItem("jug of water", "grapes");
                    sleepUntilOnClientThread(() -> Rs2Widget.getWidget(17694734) != null);
                    keyPress('1');
                    sleepUntilOnClientThread(() -> !Rs2Inventory.hasItem("jug of water"),25000);
                    return;
                } else {
                    bank();
                }
    ...
    private void bank(){
        ...
        if(Rs2Bank.isOpen()){
            ...
            if(Rs2Bank.hasItem("jug of water") &&  Rs2Bank.hasItem("grapes")) {
                ...
                sleepUntilOnClientThread(() -> Rs2Inventory.hasItem("jug of water"));
                Rs2Bank.withdrawItemX(true, "grapes", 14);
                sleepUntilOnClientThread(() -> Rs2Inventory.hasItem("grapes"));
            } else {
