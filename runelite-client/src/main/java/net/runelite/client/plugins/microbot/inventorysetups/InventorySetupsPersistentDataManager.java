...
public class InventorySetupsPersistentDataManager
{
	...
	public void updateConfig(boolean updateSetups, boolean updateSections)
	{
		if (updateSetups)
		{
			...
			configManager.setConfiguration(MInventorySetupsPlugin.CONFIG_GROUP, CONFIG_KEY_SETUPS_V2, data);
		}

		if (updateSections)
		{
			final String jsonSections = gson.toJson(sections);
			configManager.setConfiguration(MInventorySetupsPlugin.CONFIG_GROUP, CONFIG_KEY_SECTIONS, jsonSections);
		}
	...
	{
		final String storedData = configManager.getConfiguration(MInventorySetupsPlugin.CONFIG_GROUP, configKey);
	...
	{
		String hasMigratedToV2 = configManager.getConfiguration(MInventorySetupsPlugin.CONFIG_GROUP, CONFIG_KEY_SETUPS_MIGRATED_V2);
		if (Strings.isNullOrEmpty(hasMigratedToV2))
		{
			...
			inventorySetups.clear();
			configManager.setConfiguration(MInventorySetupsPlugin.CONFIG_GROUP, CONFIG_KEY_SETUPS_MIGRATED_V2, "True");
		}
