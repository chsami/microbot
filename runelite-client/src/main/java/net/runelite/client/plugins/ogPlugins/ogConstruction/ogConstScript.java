...
public class ogConstScript extends Script {
    ...
    private void callDelay(){
        if(Random.random(1,this.delayChance) == 3) {
            ...
            System.out.println("Delay range of: " + delayGeneratedMin + "-" + delayGeneratedMax + " generated a delay of " + delayGenerated + "ms.");
            sleep(delayGenerated);
        } else {
            int delayGenerated = Random.random(delayMin,delayMax);
            System.out.println("Delay range of: " + this.delayMin + "-" + this.delayMax + " generated a delay of " + delayGenerated + "ms.");
            sleep(delayGenerated);
        }
    }
    ...
    private void callAFK(){
        if(Random.random(0,this.afkChance) == 3) {
            ...
            System.out.println("AFK range of: " + this.afkMin + "-" + this.afkMax + " generated a delay of " + afkGenerated + "minutes.");
            sleep(afkGenerated);
        }
    }
    ...
    public boolean run(ogConstConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            if (!super.run()) return;
            try {
                System.out.println("Script rerunning");
                setSettings(config);
                calcState();
    ...
    private void setSettings(ogConstConfig config){
        System.out.println("Checking for updates in Settings");
        if(this.currentconfig != config){this.currentconfig = config; System.out.println("Updated main config"); }
        if(this.delayMin != config.delayMin() ){ this.delayMin = config.delayMin(); System.out.println("Updated Delay Min: " + this.delayMin); }
        if(this.delayMax != config.delayMax()){ this.delayMax = config.delayMax(); System.out.println("Updated Delay Max: " + this.delayMax); }
        if(this.delayChance != config.delayChance()){ this.delayChance = config.delayChance(); System.out.println("Updated Delay Deviation Chance: " + this.delayChance); }
        if(this.afkMin != config.afkMin()){ this.afkMin = config.afkMin(); System.out.println("Updated AFK Min: " + this.afkMin); }
        if(this.afkMax != config.afkMax()){ this.afkMax = config.afkMax(); System.out.println("Updated AFK Max: " + this.afkMax); }
        if(this.afkChance != config.afkChance()){ this.afkChance = config.afkChance(); System.out.println("Updated AFK Chance: " + this.afkChance);}
        if(this.furniture != config.selectedFurniture()){ this.furniture = config.selectedFurniture(); System.out.println("Updated selected furniture: " + this.furniture.getName()); }
        if(this.butler != config.selectedButler()){ this.butler = config.selectedButler(); System.out.println("Updated selected butler: " + this.butler.getName()); }
        if(this.useServentsBag != config.useMoneyBag()){ this.useServentsBag = config.useMoneyBag(); System.out.println("Updated use Servants Moneybag:" + this.useServentsBag); }
        if(this.moneyBagRefillThreshold != config.getMinMoneybagAmount()){ this.moneyBagRefillThreshold = config.getMinMoneybagAmount(); System.out.println("Updated Servants Moneybag Refill Amount: " + this.moneyBagRefillThreshold); }
        if(this.logging != config.verboseLogging()){ System.out.println("Updated Verbose Logging: " + config.verboseLogging() ); this.logging = config.verboseLogging(); }
    }

    private void build(){
        Microbot.status = "Building " + furniture.getName();
        System.out.println("===========================BUILD FUNCTION CALLED===========================");
        ...
        if(Rs2Widget.getWidget(30015493) != null){
            callDelay();
            System.out.println("Selecting build option");
    ...
    private void destroy(){
        ...
        System.out.println("Destroying:" + furniture.getBuiltID());
        if(furniture.getBuiltID() == ObjectID.DOOR_13344 &&
            ...
            System.out.println("Sleeping until: checkButlerNearPlayer");
        ...
        System.out.println("Sleeping until: Really Remove it?, 800-1200");
        ...
        if(Rs2Widget.findWidget("Really remove it?") != null){
            if(Random.random(1,29) == 3){
                ...
                System.out.println("Selecting Yes");
            }
            else{
                callDelay();
    ...
    private void fillMoneyBag(){
        ...
        System.out.println("===========================FILL MONEYBAG FUNCTION CALLED===========================");
    ...
    private void sendButler(){
        ...
        if(!checkIfButlerHere() ||
            ...
            if(Rs2Widget.findWidget("Master, I have returned with") != null){
                Rs2Keyboard.keyPress(KeyEvent.VK_SPACE);return;}
            System.out.println("Sleeping until butler near player, 1000 - 2000");
        ...
        System.out.println("Looking for: Repeat last task?");
    ...
    private void enterPlayerHouse() {
        System.out.println("Attempting to enter player house in build mode");
        if (Rs2GameObject.interact("Portal", "Build mode")) {
            sleepUntil(this::isInPlayerHouse, Random.random(5000, 10000));
        } else {
            System.out.println("Failed to find or interact with the Portal");
        }
    }
    ...
    private void enableBuildingMode() {
        ...
        System.out.println("===========================ENABLE BUILDING MODE FUNCTION CALLED===========================");
        if (!isInPlayerHouse()) {
            enterPlayerHouse();
        } else {
            navigateToHouseSettings();
            if (Rs2Widget.getWidget(24248325) != null) {
                callDelay();
                System.out.println("Clicking Enable Building Mode");
    ...
    private void navigateToHouseSettings() {
        ...
        if(Rs2Widget.getWidget(7602207) == null){
            ...
            System.out.println("Selected Correct Settings Tab");
            sleepUntil(()-> Rs2Widget.getWidget(7602207) != null);
        }
        if(Rs2Widget.getWidget(24248342) == null){
            sleep(30,40);
            Rs2Widget.clickWidget(7602207);
            System.out.println("Clicked House Options");
    ...
    public void onGameObjectSpawned(GameObjectSpawned event) {
        try{
            if(event.getGameObject().getId() == furniture.getBuiltID() || event.getGameObject().getId() == furniture.getUnBuiltID()){
                System.out.println("Game Object Spawned:" + event.getGameObject().getId());
            }
        } catch (Exception e){}
    }
    ...
    public void onGameTick(GameTick gameTick) {
        ...
        System.out.println("Game Tick: " + currentGameTick);
        //callAFK();
    }
    ...
    private void calcState(){
        ...
        System.out.println("===========================CALC STATE FUNCTION CALLED===========================");
        if(Microbot.getClientThread().runOnClientThread(() -> Microbot.getClient().getVarbitValue(2176)) == 0){
        ...
        System.out.println("Calculating State: " + status.name());
        System.out.println("Current Game Tick: " + currentGameTick);
        System.out.println("Game Tick last sent butler: " + gameTickLastSentButler);
        System.out.println("Should send butler: " + (currentGameTick > gameTickLastSentButler + (butler.getTicksNeededToBank() + 2)));
        System.out.println("Calculating Noted Planks: " + Rs2Inventory.hasItemAmount(furniture.getNotedPlankNameNeeded(), furniture.getPlankAmountNeeded(), true));
        System.out.println("Calculating Coins: " + Rs2Inventory.hasItemAmount("Coins",10000, true));
        System.out.println("Need to refill money bag?: " + (moneyBagRefillThreshold >= coinsLeftInMoneyBag));
        System.out.println("Refill amount: " + moneyBagRefillThreshold);
        System.out.println("Money Bag amount: " + coinsLeftInMoneyBag);
    }
    }