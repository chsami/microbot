...
class AutoVorkathPlugin : Plugin() {
    ...
    private var foods: List<Rs2Item>? = null
    ...
    private fun run() {
        while (running) {
            ...
            if (vorkath != null && vorkath.isInteracting) {
                ...
                centerTile =
                    WorldPoint(vorkath.worldLocation.x + 3, vorkath.worldLocation.y - 5, vorkath.worldLocation.plane)
                ...
                when (botState) {
                    ...
                    State.EAT -> if (foods?.size!! > 0) {
                        Rs2Inventory.interact(foods!![0], "eat");
                    ...
                    State.PRAYER -> if (Rs2Inventory.get("prayer") != null) {
                        Rs2Inventory.interact("prayer")
                        botState = previousBotState
                    } else {
                    ...
                    State.ANTIFIRE_POTION -> if (Rs2Inventory.get("antifire") != null) {
                    ...
                    State.ANTIVENOM -> if (Rs2Inventory.get("venom") != null) {
                        Rs2Inventory.interact("venom")
                        Rs2Player.antiVenomTime =
                            -64; //set this immediatly because the antivenom timer takes a while before it gets triggered
                        botState = previousBotState
                    } else {
        ...
        while (botState == State.ACID && previousBotState == State.ACID && (doesProjectileExistById(acidProjectileId) || doesProjectileExistById(
            ...
            if (currentPlayerLocation.y != clickedTile.y) {
                ...
                while (client.localPlayer.worldLocation != clickedTile && client.localPlayer.worldLocation.distanceTo(
                        clickedTile
                    ) > 1 && client.localPlayer.worldLocation.y == clickedTile.y && Microbot.isMoving()
                ) {
                    sleep(1)
                }
                toggle = !toggle
    ...
    private fun eatAt(health: Int) {
        if (clientThread.runOnClientThread { client.getBoostedSkillLevel(Skill.HITPOINTS) } < health && Rs2Npc.getNpc("Vorkath") != null) {
            ...
            if (food != null) {
                ...
                //println("No food found")
                // Teleport
                Rs2Inventory.use(config.TELEPORT().toString())
            }
        }
    }
