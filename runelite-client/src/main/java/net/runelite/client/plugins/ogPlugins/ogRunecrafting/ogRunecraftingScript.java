...
public class ogRunecraftingScript extends Script {
    ...
    private void combinationCraft(){
        Rs2Inventory.useItemOnObjectFast(config.selectRuneToMake().getPrimaryRequiredRune(),config.selectAlter().getAlterID());
    ...
    private void grabRunes() {
        ...
        if(!Rs2Bank.isOpen()){
            ...
            if(Rs2Inventory.hasItem("lava rune")){
                ...
                //Rs2Inventory.useItemUnsafe("Lava rune");
            ...
            if(bindingNeckAmmount < 4){
                //Rs2Bank.withdrawItem(false, "Binding Necklace" );sleepUntil(() -> Rs2Inventory.hasItem(ItemID.BINDING_NECKLACE), Random.random(601,620));
            ...
            Rs2Widget.clickChildWidget(786445,477);
            sleep(240,300);
//            while(!Rs2Inventory.hasItemAmount(config.selectRuneToMake().getEssenceTypeRequired(), 23) && pouchMaxCycle < 2){
//                Rs2Widget.clickChildWidget(786445,474);
//                //sleepUntil(()-> Rs2Inventory.hasItemAmount(config.selectRuneToMake().getEssenceTypeRequired(), 23));
//                Rs2Inventory.useItemAction("Colossal pouch","Fill");
//                sleep(200,300);
//                pouchMaxCycle++;
//            } pouchMaxCycle = 0;
        }
    ...
    private void walkToAlter() {
        ...
        if(Rs2Inventory.hasItem(ItemID.BINDING_NECKLACE)){
            Rs2Inventory.useItem("Binding Necklace");
            sleepUntil(()-> Rs2Widget.findWidget("Destroy necklace of binding?") != null);
            Rs2Widget.clickWidget("Yes");
            bindingNeckAmmount = getBindingNeckAmmount();
        }
        if(Rs2Inventory.hasItem("Stamina potion(1)")){if(Tab.getCurrentTab() != InterfaceTab.INVENTORY){Tab.switchToInventoryTab(); sleepUntil(()-> Tab.getCurrentTab() == InterfaceTab.INVENTORY);}
            Rs2Inventory.useItemAction("Stamina potion(1)","drink");}
    ...
    private void craftRunes(){
        ...
        if(config.selectRuneToMake().isCombinationRune() && hasEssence()){
            while(hasEssence() && pouchMaxCycle < 3){
                ...
                Rs2Inventory.useItemAction("Colossal pouch","Empty");
            } pouchMaxCycle = 0;
